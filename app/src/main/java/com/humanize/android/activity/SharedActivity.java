...
public class SharedActivity extends AppCompatActivity {
    ...
    private void initialize() {
        ...
        toolbarText.setText(StringConstants.SHARED_ARTICLES);
        ...
        try {
            if (SharedPreferencesService.getInstance().getString(Config.JSON_SHARED_CONTENTS) != null) {
                Contents contents = jsonParserService.fromJson(SharedPreferencesService.getInstance().getString(Config.JSON_SHARED_CONTENTS), Contents.class);

                if (contents.getContents().size() == 0) {
                    NoSharedFragment noSharedFragment = new NoSharedFragment();
                    noSharedFragment.show(SharedActivity.this.getFragmentManager(), "");
                }

                SharedActivity.contents = contents;
                contentRecyclerViewAdapter.setContents(SharedActivity.contents.getContents());
    ...
    public static class NoSharedFragment extends DialogFragment {

        @Override
        public Dialog onCreateDialog(Bundle savedInstanceState) {

            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            LinearLayout linearLayout = (LinearLayout) getActivity().getLayoutInflater().inflate(R.layout.dialog_no_shared, null);
